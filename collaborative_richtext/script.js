...
(function() {
  var CollaborativeRichText, Overlay, random_character, random_id, register_collaborative_fields;
  ...
  Overlay = (function() {
    ...
    Overlay.prototype.collaborative_fields = ['start', 'end', 'attribute'];

    Overlay.prototype.initialize = function(arg) {
    ...
    Overlay.prototype.loaded = function() {
      return this.model = gapi.drive.realtime.custom.getModel(this);
    };
    ...
    Overlay.prototype.collides = function(index) {
      return this.get_start() <= index && this.get_end() >= index;
    };

    Overlay.prototype.collides_attribute = function(index, attribute) {
      return attribute === this.attribute && this.collides(index);
    };

    Overlay.prototype.is_inside = function(start_index, end_index) {
      return this.get_start() >= start_index && this.get_end() <= end_index;
    };
  ...
  CollaborativeRichText = (function() {
    ...
    CollaborativeRichText.prototype.collaborative_fields = ['str', 'overlays'];

    CollaborativeRichText.prototype.initialize = function() {
      this.model = gapi.drive.realtime.custom.getModel(this);
      this.str = this.model.createString();
      return this.overlays = this.model.createList();
    };

    CollaborativeRichText.prototype.loaded = function() {
      return this.model = gapi.drive.realtime.custom.getModel(this);
    };
    ...
    CollaborativeRichText.prototype.delete_overlay_range = function(start_index, end_index) {
      ...
      deletable_overlays = this.overlays.asArray().filter(function(overlay) {
        return overlay.is_inside(start_index, end_index);
      });
  ...
  register_collaborative_fields = function(cls, name) {
    var field, i, len, ref, results;
    gapi.drive.realtime.custom.registerType(cls, name);
    gapi.drive.realtime.custom.setInitializer(cls, cls.prototype.initialize);
    gapi.drive.realtime.custom.setOnLoaded(cls, cls.prototype.loaded);
    ref = cls.prototype.collaborative_fields;
    results = [];
    for (i = 0, len = ref.length; i < len; i++) {
      field = ref[i];
      results.push(cls.prototype[field] = gapi.drive.realtime.custom.collaborativeField(field));
    }
    return results;
  };
  ...
  window.setup_richtext = function() {
    ...
    return register_collaborative_fields(Overlay, 'CollaborativeRichTextOverlay');
  };
